...
function tickWorld(){
    let horz = ( -key(Players[0].controlScheme["left"]) + key(Players[0].controlScheme["right"]) ) * tileSize;
    let vert = ( -key(Players[0].controlScheme["up"])   + key(Players[0].controlScheme["down"])  ) * tileSize;
    
    let lastX = Players[0].real.x;
    let lastY = Players[0].real.y;
    Players[0].real.x += horz;
    Players[0].real.y += vert;

    if(Players[0].hitbox){
        Players[0].real.x = lastX;
        Players[0].real.y = lastY;
    }

    Players[0].real.x = Math.round(Players[0].real.x / 19) * 19;
    Players[0].real.y = Math.round(Players[0].real.y / 19) * 19
}
...
function canPlayerMove(player2test = Players[0]){
    if( Math.abs(-key(player2test.controlScheme["left"]) + key(player2test.controlScheme["right"])) >= 1 ){
        return true;
    }
    if( Math.abs(-key(player2test.controlScheme["up"]) + key(player2test.controlScheme["down"])) >= 1 ){
        return true;
    }
    return false;
}
